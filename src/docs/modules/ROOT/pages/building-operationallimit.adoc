...
The Operational Limits are used to define the voltage, current and power limits (maximum or minimum) that can flow through a piece of equipment. For example, the active power limit is used to define the maximum active power that can flow through a piece of equipment.
...
* https://modellen.netbeheernederland.nl/tc57cim/cim17/schema/class/ActivePowerLimit[cim:ActivePowerLimit]
* https://modellen.netbeheernederland.nl/tc57cim/cim17/schema/class/ApparentPowerLimit[cim:ApparentPowerLimit]
* https://modellen.netbeheernederland.nl/tc57cim/cim17/schema/class/CurrentLimit[cim:CurrentLimit]
* https://modellen.netbeheernederland.nl/tc57cim/cim17/schema/class/VoltageLimit[cim:VoltageLimit]

=== Grouping Operational Limits
There are two structures for operational limits: the OperationalLimitSet and the OperationalLimitType. The https://modellen.netbeheernederland.nl/tc57cim/cim17/schema/class/OperationalLimitSet.html[cim:OperationalLimitSet] is used to group together different types of operational limits that apply to a specific piece of equipment, while the https://modellen.netbeheernederland.nl/tc57cim/cim17/schema/class/OperationalLimitType.html[cim:OperationalLimitType] is used to define the specific type of limit (e.g., duration and directionkind (abs, max, min)) that applies to the equipment.

_* OperationalLimitType not yet part of the NBNL EQ profile_

=== OperationalLimitSet
The OperationalLimitSet object is used to group together different types of operational limits. The https://modellen.netbeheernederland.nl/tc57cim/cim17/schema/class/OperationalLimitSet.html[cim:OperationalLimitSet] is associated with a https://modellen.netbeheernederland.nl/tc57cim/cim17/schema/class/Terminal.html[cim:Terminal] of a for example https://modellen.netbeheernederland.nl/tc57cim/cim17/schema/class/PowerTransformerEnd.html[cim:PowerTransformerEnd].
Which means that the operational limits defined in the OperationalLimitSet apply to the (power) flow through the terminal and equipment combination.

=== Redundancy Constraints (e.g., n-1)
Redundancy is an important aspect of the operational limits. The operational limits can be defined for a single piece of equipment, such as a transformer, or for a set of equipment, such as a group of transformers. When set for a group of equipments it can be used to define redundancy, like n-1 configurations. The operational limit is used to define an N-1 constraint.
...
By applying the flow direction (true or false), the value of the operational limit is always positive, regardless of the direction of the flow. Thus, feed-in or consumption is determined by the flow direction, the equipment terminal used, and the PositiveFlowIn attribute. This is important in situations where, for example due to policy, the limit is not identical in both directions of the conducting equipment.

*Definition: positiveFlowIn*

_If true then the measurement is an active power, reactive power or current with the convention that a positive value measured at the Terminal means power is flowing into the related PowerSystemResource._
...
*The terminal of the equipment, in combination with the True of False value of the attribute extension nl:PositiveFlowIn and the fact that the limit value is positive or negative define the direction of the power flow.*
